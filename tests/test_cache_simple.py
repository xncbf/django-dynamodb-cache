...
def test_get_delete_many(cache):
...
def test_incr_dec_version(cache):
...
@patch("warnings.warn")
def test_verify_key(mock_warn, cache):
    BIG = "0" * 251
...
def test_incr_decr_value(cache):
    cache.set("i", 10)
    cache.decr("i", 42)
    value = cache.get("i")
    assert value == -32
